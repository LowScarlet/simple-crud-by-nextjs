...
export const exportKategoriSimpananToExcel = async (data: (Prisma.KategoriSimpananCreateInput & { id: number })[]) => {
  if (!data) return;

  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('Data Kategori Simpanan');

  // Set title
  worksheet.mergeCells('A1:C1');
  const titleCell = worksheet.getCell('A1');
  titleCell.value = 'Laporan Data Kategori Simpanan';
  ...
  a.href = url;
  a.download = `Laporan_Data_Kategori_Simpanan_${new Date().toISOString().split('T')[0]}.xlsx`;
  a.click();
